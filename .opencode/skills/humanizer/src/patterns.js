...
const patterns = [
...

  {
    ...
    category: 'style',
    description:
      'Unicode curly quotes (\u201C\u201D\u2018\u2019) instead of straight quotes. Context-dependent: normal in published books and word processors, potentially suspicious in plain-text environments.',
    weight: 1,
    detect(text) {
      const matches = text.match(/[\u201C\u201D\u2018\u2019]/g);
      if (matches && matches.length >= 3) {
        return [{
          match: `${matches.length} curly quotes found`,
          index: text.search(/[\u201C\u201D\u2018\u2019]/),
          line: 1,
          column: 1,
          suggestion: 'Replace curly quotes with straight quotes if in a plain-text context. Not meaningful in published or typeset content.',
          confidence: 'low',
        }];
      }
      return [];
  ...
  {
    id: 30,
    name: 'Biology/ecosystem over-emphasis',
    category: 'content',
    description:
      'LLMs over-emphasize connections to broader ecosystems, conservation status, and cultural heritage when writing about species or nature. Boilerplate about "playing a role in the ecosystem" or "preserving this species is vital" even when the status is unknown.',
    weight: 3,
    detect(text) {
      const results = [];

      // Ecosystem role boilerplate
      const ecosystemRole = [
        /\bplays? a (vital|crucial|important|significant|key|essential|fundamental) role in (its|the|their|the local|the broader) (ecosystem|habitat|environment)\b/gi,
        /\bcontributes? to the (broader )?(ecosystem|environment|habitat|ecological)\b/gi,
        /\b(ecosystem|habitat|environment) (is|are) (crucial|vital|essential|important) for the survival\b/gi,
        /\b(integral|essential|important) part of (its|the|their|the local) (ecosystem|habitat|environment)\b/gi,
      ];
      for (const regex of ecosystemRole) {
        results.push(
          ...findMatches(
            text,
            regex,
            'Remove generic ecosystem boilerplate. State a specific ecological interaction instead.',
            'high',
          ),
        );
      }

      // Conservation hedging — mentions unknown status then adds significance anyway
      const conservationHedging = [
        /\bconservation status (is|remains) (unknown|not assessed|not evaluated|data deficient|unclear)\b[^.]{0,100}\b(however|but|nonetheless|nevertheless)\b/gi,
        /\b(however|but|nonetheless|nevertheless),? (the )?(general|overall) health of the (ecosystem|habitat)\b/gi,
        /\bfactors? such as .{10,80} could potentially impact\b/gi,
        /\bcurrently,? there is no (specific|formal|official) conservation (assessment|status|evaluation)\b[^.]{0,100}\b(however|but)\b/gi,
      ];
      for (const regex of conservationHedging) {
        results.push(
          ...findMatches(
            text,
            regex,
            'Remove conservation hedging. If the status is unknown, state that without adding hypothetical significance.',
            'high',
          ),
        );
      }
      ...
      const culturalPadding = [
        /\bcontributes? to .{0,40} (rich )?cultural heritage\b/gi,
        /\bsustaining the cultural traditions\b/gi,
        /\bpreserving this (endemic|native|unique) species is (vital|crucial|important|essential)\b/gi,
        /\bdeeply (rooted|embedded|ingrained) in the (culture|heritage|traditions)\b/gi,
      ];
      for (const regex of culturalPadding) {
        results.push(
          ...findMatches(
            text,
            regex,
            'Remove cultural heritage padding. If there is a specific cultural connection, name it.',
            'medium',
          ),
        );
      }
      ...
      const preservationBoilerplate = [
        /\bpreserv(e|ing|ation) of (this|the|these) (species|habitat|ecosystem) is (vital|crucial|important|essential|critical)\b/gi,
        /\b(general|overall) health of the (ecosystem|habitat|environment) is (crucial|vital|important|essential|critical)\b/gi,
        /\bpreserving (biodiversity|natural habitats|wildlife|ecosystems) is (crucial|vital|essential|important)\b/gi,
      ];
      for (const regex of preservationBoilerplate) {
        results.push(
          ...findMatches(
            text,
            regex,
            'Replace preservation boilerplate with a specific conservation action or fact.',
            'medium',
          ),
        );
      }
  ...
  {
    id: 31,
    name: 'Trivial-fact over-attribution',
    category: 'content',
    description:
      'LLMs cite sources for every mundane fact — "According to the 2019 annual report, the company was founded in 2005." A human would just state the fact with a simple citation. High density of attribution phrases signals AI padding.',
    weight: 3,
    detect(text) {
      const results = [];
      const words = wordCount(text);
      ...
      const attributionPatterns = [
        /\baccording to (the|a|an) .{3,50} (report|study|survey|publication|article|document|analysis|review|database|census)\b/gi,
        /\bas (reported|documented|noted|stated|described|mentioned|recorded|cited) (by|in)\b/gi,
        /\b(as of|according to) .{3,50} (data|figures|statistics|records|estimates)\b/gi,
        /\bthe .{3,50} (report|study|survey|census|analysis) (found|noted|stated|reported|indicated|revealed|showed|estimated)\b/gi,
        /\b[12][0-9]{3} (report|study|survey|publication|census|analysis) (from|by|of|conducted by)\b/gi,
        /\b(mentioned|featured|covered|profiled) in .{3,50} (coverage|article|report|publication)\b/gi,
      ];

      // Collect all attribution matches
      const allMatches = [];
      for (const regex of attributionPatterns) {
        const m = text.match(regex);
        if (m) allMatches.push(...m);
      }

      // Flag based on density
      if (allMatches.length > 0) {
        const density = allMatches.length / (words / 100); // attributions per 100 words

        if (density > 3.0) {
          // Very high density: >3 attributions per 100 words
          results.push({
            match: `${allMatches.length} attributions in ${words} words (${density.toFixed(1)}/100 words)`,
            index: 0,
            line: 1,
            column: 1,
            suggestion: 'Very high density of source attributions. Human writing typically only cites sources for non-obvious claims. Consider whether every fact needs a named source.',
            confidence: 'high',
          });
        } else if (density > 1.5 && words < 300) {
          // Moderate density in short text
          results.push({
            match: `${allMatches.length} attributions in ${words} words (${density.toFixed(1)}/100 words)`,
            index: 0,
            line: 1,
            column: 1,
            suggestion: 'High attribution density for a short text. Check whether these are all necessary or if some facts could stand on their own.',
            confidence: 'medium',
          });
        }

        // Also flag individual "as of [year]" temporal framings — a common LLM tell
        const asOfPattern = /\bas of (the )?(January|February|March|April|May|June|July|August|September|October|November|December|early|late|mid)?.{0,30}(20[0-9]{2}|19[0-9]{2})\b/gi;
        const asOfMatches = text.match(asOfPattern);
        if (asOfMatches && asOfMatches.length >= 2) {
          results.push(
            ...findMatches(
              text,
              asOfPattern,
              'Multiple "as of [date]" framings. Vary temporal references or use a single framing.',
              'medium',
            ),
          );
        }

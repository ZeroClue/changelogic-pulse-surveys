...
const { naturalize, naturalizeAuto, detectRegister } = require('./naturalize');
...
const flags = {
  ...
  naturalize: (() => {
    const idx = args.findIndex(a => a.startsWith('--naturalize'));
    if (idx === -1) return false;
    const val = args[idx].split('=')[1];
    return val || true;
  })(),
...
${color.bold('Commands:')}
  ...
  ${color.cyan('naturalize')}   Apply naturalization pass (contractions, colloquial swaps, sentence looseners)
...
${color.bold('Options:')}
  ...
  --naturalize[=level]    Apply naturalization pass (auto-detects register; override with light|medium|heavy)
...
async function main() {
  ...
  const textCommands = new Set(['analyze', 'score', 'humanize', 'naturalize', 'report', 'suggest', 'stats']);
  ...
  switch (command) {
    ...
    case 'humanize': {
      ...
      if (flags.json) {
        ...
        if (flags.autofix && result.autofix) {
          let output = result.autofix.text;
          if (flags.naturalize) {
            const explicitLevel = typeof flags.naturalize === 'string' ? flags.naturalize : null;
            if (explicitLevel) {
              output = naturalize(output, explicitLevel);
            } else {
              output = naturalizeAuto(output).text;
            }
          }
          console.log(`\n${color.bold('── AUTO-FIXED TEXT ──────────────────────────────')}\n`);
          console.log(output);
          console.log(`\n${color.dim('════════════════════════════════════════════════')}`);
        } else if (flags.naturalize) {
          const explicitLevel = typeof flags.naturalize === 'string' ? flags.naturalize : null;
          let output, intensity;
          if (explicitLevel) {
            output = naturalize(text, explicitLevel);
            intensity = explicitLevel;
          } else {
            const auto = naturalizeAuto(text);
            output = auto.text;
            intensity = auto.intensity;
          }
          console.log(`\n${color.bold(`── NATURALIZED TEXT (${intensity}) ──────────────────`)}\n`);
          console.log(output);
          console.log(`\n${color.dim('════════════════════════════════════════════════')}`);
        }
      }
    ...
    case 'naturalize': {
      ...
      if (explicitLevel) {
        result = naturalize(text, explicitLevel);
        intensity = explicitLevel;
      } else {
        const auto = naturalizeAuto(text);
        result = auto.text;
      ...
      if (flags.json) {
        console.log(JSON.stringify({ original: text, naturalized: result, intensity }));
      } else {
        console.log(`${color.dim(`── naturalized (${intensity}) ────────────────────────`)}\n`);
        console.log(result);
      }
      break;
    }
